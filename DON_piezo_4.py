...
sigma=0.10 # 10 percent Gaussian noise
u_train = u_train * (1 + sigma * np.random.randn(1000, 100))
u_test = u_test * (1 + sigma * np.random.randn(1000, 100))

############################################################################
###################### Visualizing fields ###########################
############################################################################

# Step 1: Randomly select 10 functions from the numpy array
random_indices = np.random.choice(u_train.shape[0], size=10, replace=False)
random_functions_train = u_train[random_indices, :]
random_functions_test = u_test[random_indices, :]

# Step 1: Create the figure and define the grid layout
gs1 = gridspec.GridSpec(1, 1)
ax1 = plt.subplot(gs1[:, :])



# Step 2: Plot each of the 10 randomly selected train functions in blue
for function_train in random_functions_train:
    ax1.plot(x_train, function_train, color='blue', lw=2.0, alpha=0.5, label='Train')  # Blue for train

# Step 3: Plot each of the 10 randomly selected test functions in green
for function_test in random_functions_test:
    ax1.plot(x_train, function_test, '--r', lw=2.0, alpha=0.5, label='Test')  # Green for test

# Step 4: Set axis labels with increased fontsize
ax1.set_xlabel('Time [s]', fontsize=26)
ax1.set_ylabel('Noisy outputs', fontsize=26)

# Step 5: Increase fontsize for x and y ticks
ax1.tick_params(axis='both', which='major', labelsize=26)

# Step 6: Show the legend (only one label for 'Train' and 'Test')
handles, labels = ax1.get_legend_handles_labels()
by_label = dict(zip(labels, handles))  # Ensure unique labels in the legend
ax1.legend(by_label.values(), by_label.keys(), frameon=False, loc='best', fontsize=17)

# Step 7: Apply tight layout to avoid clipping
plt.tight_layout()

# Step 8: Save and show the plot
plt.savefig("Results/output_fn_sine.png", dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig("Results/DON/DON_Exp4.png", dpi=300)
plt.show()
