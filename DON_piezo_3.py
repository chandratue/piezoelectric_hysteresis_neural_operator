...
d = np.load("Data/Exp3_Train.npz", allow_pickle=True)
v_train, x_train, u_train = d["voltage"], d["X"], d["displacement"]

d = np.load("Data/Exp3_Test.npz", allow_pickle=True)
...
random_indices = np.random.choice(v_train.shape[0], size=10, replace=False)
random_functions_train = v_train[random_indices, :]
random_functions_test = v_test[random_indices, :]
...
ax1.set_ylabel('Sampled fields', fontsize=26)
...
ax1.legend(by_label.values(), by_label.keys(), frameon=False, loc='best', fontsize=15)

# Step 7: Apply tight layout to avoid clipping
plt.tight_layout()

# Step 8: Save and show the plot
plt.savefig("Results/input_fn_nonsine.png", dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig("Results/DON/DON_Exp3.png", dpi=300)
plt.show()
